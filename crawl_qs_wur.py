...
def crawl_data(api_token, output_dir):
    ...
    last_univ_id, last_academic_reputation_id, last_citations_per_faculty_id, last_employer_reputation_id, last_employment_outcomes_id, last_faculty_student_ratio_id, last_international_faculty_ratio_id, last_international_research_network_id, last_international_students_ratio_id, last_sustainability_id, last_year = get_last_ids_and_year(cursor)

    url_template = f"https://www.topuniversities.com/rankings/endpoint?nid={api_token}&page={{}}&items_per_page=15&tab=indicators"
   ...
   # Save DataFrames to CSV in specified directory
    df_all.to_csv(f"{output_dir}/univ_info.csv", index=False)
    df_academic_reputation.to_csv(f"{output_dir}/academic_reputation.csv", index=False)
    df_citations_per_faculty.to_csv(f"{output_dir}/citations_per_faculty.csv", index=False)
    df_employer_reputation.to_csv(f"{output_dir}/employer_reputation.csv", index=False)
    df_employment_outcomes.to_csv(f"{output_dir}/employment_outcomes.csv", index=False)
    df_faculty_student_ratio.to_csv(f"{output_dir}/faculty_student_ratio.csv", index=False)
    df_international_faculty_ratio.to_csv(f"{output_dir}/international_faculty_ratio.csv", index=False)
    df_international_research_network.to_csv(f"{output_dir}/international_research_network.csv", index=False)
    df_international_students_ratio.to_csv(f"{output_dir}/international_students_ratio.csv", index=False)
    df_sustainability.to_csv(f"{output_dir}/sustainability.csv", index=False)
    df_overall.to_csv(f"{output_dir}/overall.csv", index=False)
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        print("Usage: python crawl_qs_wur.py <API_TOKEN> <OUTPUT_DIR>")
        sys.exit(1)
    
    token = sys.argv[1]
    output_directory = sys.argv[2]
    crawl_data(token, output_directory)
